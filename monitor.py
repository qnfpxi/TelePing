    ...
    for attempt in range(retries):
        ...
        try:
            ...
            test_msg = json.dumps({
                ...
                "pro_ids": node_config["pro_ids"]     # 省份ID数组（官方API参数）
            })
            logging.info(f"17CE 测速请求（{len(node_config['pro_ids'])}个核心省份，每省{node_config['num']}个节点）: {test_msg}")
            ws.send(test_msg)

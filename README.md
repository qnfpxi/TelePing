...
- **分布式监控**：调用 17CE API，覆盖8个核心省份（北京、上海、广东、浙江、江苏、四川、湖北、陕西）
...
- **💰 节约积分**：8个核心省份配置，每次约20-30个节点，积分消耗低
...
- `core_provinces`：核心省份ID列表（默认8个：北京、上海、广东、浙江、江苏、四川、湖北、陕西）
- `num`：**每个省份分配的节点数**（默认 3）
...
- 实际返回节点数：20-30 个/次（取决于 17CE 节点可用性）

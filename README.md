...
- **分布式监控**：调用 17CE API，覆盖14个核心省份（4直辖市 + 10个经济/交通枢纽省份）
...
- **💰 节约积分**：14个核心省份配置，每次约40-55个节点，全面覆盖且积分消耗合理
...
- `core_provinces`：核心省份ID列表（默认14个）
  - **4个直辖市**：北京、上海、重庆、天津
  - **东部沿海**：广东、浙江、江苏、山东、福建
  - **中西部枢纽**：四川、湖北、河南、湖南、陕西
- `num`：**每个省份分配的节点数**（默认 3）
...
**关键技术说明**：
- 使用 `pro_ids`（省份ID数组）**这是官方API参数**
- `city_ids` 不是官方API参数，已移除
- 实际返回节点数：40-55 个/次（取决于 17CE 节点可用性）
- 参数格式：必须使用**数组**，不能用逗号分隔字符串

**积分消耗**：
- 14个核心省份配置：~40-55 节点/次
- **覆盖全国主要地区（4直辖市+10大经济省份），积分消耗合理** ✅

...
- **分布式监控**：调用 17CE API，覆盖全国 29 个省份 + 33 个主要城市，约 5-20 个节点/次（取决于节点可用性）
...
- **💰 节约积分**：pro_ids + city_ids 组合配置，每次仅 5-20 积分（相比旧配置节约 99%+）
...
- `pro_ids`：省份ID数组（从 MAJOR_CITIES 自动提取，约29个省份）
- `city_ids`：城市ID数组（从 MAJOR_CITIES 自动提取，33个城市）
- `num`：**每个城市分配的节点数**（默认 2）
...
**关键技术说明**：
- 使用 `pro_ids + city_ids` 组合方式（官方 API 要求）
- `pro_ids` 定义省份范围，`city_ids` 在范围内筛选城市
- 实际返回节点数：5-20 个/次（取决于 17CE 节点可用性）
- 参数格式：必须使用**数组**，不能用逗号分隔字符串

**积分优化效果**：
- 旧配置（省份级）：~700 积分/次
- 新配置（pro_ids + city_ids）：~5-20 积分/次
- **节约比例：99%+** ✅

**为什么节点数少于预期？**
- 17CE 节点池有限，不是所有城市/运营商组合都有可用节点
- 但已覆盖主要运营商（电信/联通/移动）和多个地区
- 5-20个节点足够监控使用，且积分消耗极低

...
- **分布式监控**：调用 17CE API，覆盖全国 29 个省份，预计 174 个节点（29省 × 1节点/省 × 2类型 × 3运营商）
...
- **💰 节约积分**：`num=1` 配置优化，每次检测约 174 个节点（可根据账户积分调整）
...
- `num`：**⚠️ 每个省份分配的节点数**（不是总节点数！）
  - 默认值：`1`（节约积分）
  - 建议值：`1-3`（根据账户积分余额调整）
  - 说明：实际节点数 = 省份数 × num × 节点类型数 × 运营商数
  - 示例：29省 × 1 × 2类型 × 3运营商 = 174个节点/次
- `nodetype`：节点类型（1=IDC, 2=路由器）
- `isps`：运营商过滤（1=电信, 2=联通, 7=移动）
- `areas`：地区过滤（1=大陆）

**积分优化建议**：
- 日常监控：`num=1`（充分覆盖，节约积分）
- 深度诊断：临时调整为 `num=2-3`（更多节点，更准确）
